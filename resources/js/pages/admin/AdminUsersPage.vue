<template>
    <div>
        ...
        <div class="mb-4">
            <input v-model="search" type="text" placeholder="Search users..." class="w-full max-w-sm rounded-md border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-white placeholder-gray-400 focus:border-indigo-500 focus:outline-none" @input="debouncedFetch" />
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase">Tenants</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase">Last Login</th>
                        <th class="px-6 py-3"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-700">
                    <tr v-if="isLoading"><td colspan="6" class="px-6 py-8 text-center text-sm text-gray-400">Loading...</td></tr>
                    <tr v-else-if="users.length === 0"><td colspan="6" class="px-6 py-8 text-center text-sm text-gray-400">No users.</td></tr>
                    <tr v-for="u in users" :key="u.id" class="hover:bg-gray-700/50">
                        <td class="px-6 py-4 text-sm font-medium text-white">
                            <router-link :to="`/admin/users/${u.id}`" class="hover:underline">{{ u.name }}</router-link>
                            <span v-if="u.is_super_admin" class="ml-1 text-xs text-indigo-400">(admin)</span>
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-400">{{ u.last_login_at ? formatDate(u.last_login_at) : '—' }}</td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <button v-if="u.locked_at" @click="unlockUser(u)" class="text-xs text-green-400 hover:text-green-300">Unlock</button>
                            <button v-else @click="lockUser(u)" class="text-xs text-red-400 hover:text-red-300">Lock</button>
                            <button @click="impersonateUser(u)" class="text-xs text-indigo-400 hover:text-indigo-300">Impersonate</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div v-if="pagination.lastPage > 1" class="flex items-center justify-between mt-4">
            ...
            <div class="flex gap-2">
                <button :disabled="pagination.currentPage <= 1" @click="fetchUsers(pagination.currentPage - 1)" class="px-3 py-1 text-sm border border-gray-600 rounded-md text-gray-300 disabled:opacity-50">Previous</button>
                <button :disabled="pagination.currentPage >= pagination.lastPage" @click="fetchUsers(pagination.currentPage + 1)" class="px-3 py-1 text-sm border border-gray-600 rounded-md text-gray-300 disabled:opacity-50">Next</button>
            </div>
        </div>
    </div>
</template>
