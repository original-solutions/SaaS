<template>
    <div>
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <button v-if="failedJobs.length > 0" @click="retryAll" class="px-3 py-1.5 text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded-md">Retry All</button>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase">Queue</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase">Exception</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase">Failed At</th>
                        <th class="px-6 py-3"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-700">
                    <tr v-if="isLoading"><td colspan="5" class="px-6 py-8 text-center text-sm text-gray-400">Loading...</td></tr>
                    <tr v-else-if="failedJobs.length === 0"><td colspan="5" class="px-6 py-8 text-center text-sm text-gray-400">No failed jobs.</td></tr>
                    <tr v-for="j in failedJobs" :key="j.id" class="hover:bg-gray-700/50">
                        ...
                        <td class="px-6 py-4 text-sm text-red-400 truncate max-w-xs" :title="j.exception">{{ j.exception?.substring(0, 80) }}...</td>
                        <td class="px-6 py-4 text-sm text-gray-400">{{ formatDate(j.failed_at) }}</td>
                        <td class="px-6 py-4 text-right">
                            <button @click="retryJob(j.id)" class="text-xs text-indigo-400 hover:text-indigo-300">Retry</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
async function retryJob(id: string): Promise<void> {
    ...
    } catch { notifications.error('Failed to retry job.'); }
}
...
async function retryAll(): Promise<void> {
    ...
    } catch { notifications.error('Failed to retry all.'); }
}
