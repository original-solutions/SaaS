<template>
    <div>
        ...
        <!-- Search -->
        <div class="mb-4">
            <input v-model="search" type="text" placeholder="Search tenants..." class="w-full max-w-sm rounded-md border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-white placeholder-gray-400 focus:border-indigo-500 focus:outline-none" @input="debouncedFetch" />
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase">Customers</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr v-else-if="tenants.length === 0"><td colspan="6" class="px-6 py-8 text-center text-sm text-gray-400">No tenants.</td></tr>
                    <tr v-for="t in tenants" :key="t.id" class="hover:bg-gray-700/50">
                        <td class="px-6 py-4 text-sm font-medium text-white">
                            <router-link :to="`/admin/tenants/${t.id}`" class="hover:underline">{{ t.name }}</router-link>
                        </td>
                        <td class="px-6 py-4">
                            <span :class="statusClass(t.status)" class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium">{{ t.status }}</span>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-300">{{ t.users_count ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-300">{{ t.customers_count ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-400">{{ formatDate(t.created_at) }}</td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <button v-if="t.disabled_at" @click="enableTenant(t)" class="text-xs text-green-400 hover:text-green-300">Enable</button>
                            <button v-else @click="disableTenant(t)" class="text-xs text-red-400 hover:text-red-300">Disable</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div v-if="pagination.lastPage > 1" class="flex items-center justify-between mt-4">
            ...
            <div class="flex gap-2">
                <button :disabled="pagination.currentPage <= 1" @click="fetchTenants(pagination.currentPage - 1)" class="px-3 py-1 text-sm border border-gray-600 rounded-md text-gray-300 disabled:opacity-50">Previous</button>
                <button :disabled="pagination.currentPage >= pagination.lastPage" @click="fetchTenants(pagination.currentPage + 1)" class="px-3 py-1 text-sm border border-gray-600 rounded-md text-gray-300 disabled:opacity-50">Next</button>
            </div>
        </div>
    </div>
</template>
