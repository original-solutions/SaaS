<template>
    <div>
        <div class="flex items-center justify-between mb-6">
            ...
            <label v-if="tenantStore.canWrite" class="inline-flex items-center gap-2 px-4 py-2 text-sm bg-gray-900 text-white rounded-md hover:bg-gray-800 cursor-pointer">
                ...
                New Import
                <input type="file" accept=".csv,.xlsx" class="hidden" @change="startImport" />
            </label>
        </div>

        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div v-if="isLoading" class="p-6 text-center text-sm text-gray-500">Loading...</div>
            <div v-else-if="imports.length === 0" class="p-6 text-center text-sm text-gray-500">No imports yet.</div>
            ...
            <table v-else class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    <tr v-for="imp in imports" :key="imp.id" class="hover:bg-gray-50">
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ imp.file_name }}</td>
                        <td class="px-6 py-4 text-sm text-gray-500 capitalize">{{ imp.entity_type }}</td>
                        <td class="px-6 py-4">
                            <span :class="statusClass(imp.status)" class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium">
                                {{ imp.status }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-500">
                            <template v-if="imp.total_rows">
                                ...
                                <span v-if="imp.failed_rows" class="text-red-500">({{ imp.failed_rows }} failed)</span>
                            </template>
                            <template v-else>—</template>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-500">{{ formatDate(imp.created_at) }}</td>
                        <td class="px-6 py-4 text-right">
                            <button v-if="imp.status === 'failed'" @click="retryImport(imp.id)" class="text-sm text-indigo-600 hover:text-indigo-800">Retry</button>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <div v-if="pagination.lastPage > 1" class="px-4 py-3 border-t border-gray-200 flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    <button :disabled="pagination.currentPage <= 1" @click="loadImports(pagination.currentPage - 1)" class="px-3 py-1 text-sm border rounded-md disabled:opacity-50">Previous</button>
                    <button :disabled="pagination.currentPage >= pagination.lastPage" @click="loadImports(pagination.currentPage + 1)" class="px-3 py-1 text-sm border rounded-md disabled:opacity-50">Next</button>
                </div>
            </div>
        </div>
    </div>
