<template>
    <div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">{{ tenantStore.current?.name }}</h2>
            ...
            <table v-else class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    <tr v-for="m in members" :key="m.id">
                        ...
                        <td class="px-4 py-3">
                            <select v-if="tenantStore.isOwner && m.id !== authStore.user?.id" v-model="m.role" @change="updateRole(m)" class="text-xs border-gray-300 rounded-md">
                        ...
                        <td v-if="tenantStore.isOwner" class="px-4 py-3 text-right">
                            <button v-if="m.id !== authStore.user?.id" @click="removeMember(m)" class="text-xs text-red-600 hover:text-red-700">Remove</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <!-- Invite Member -->
        <div v-if="tenantStore.isOwner" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <form @submit.prevent="sendInvite" class="flex flex-wrap items-end gap-3">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <input v-model="inviteForm.email" type="email" required class="w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Role</label>
                    <select v-model="inviteForm.role" class="rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="member">Member</option>
                        <option value="readonly">Read Only</option>
                    </select>
                </div>
                <button type="submit" :disabled="inviteSending" class="px-4 py-2 text-sm bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50">
        ...
        <!-- Pending Invitations -->
        <div v-if="tenantStore.isOwner && pendingInvites.length > 0" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">Pending Invitations</h2>
            <div v-for="inv in pendingInvites" :key="inv.id" class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                <div>
                    <p class="text-sm text-gray-900">{{ inv.email }}</p>
                    <p class="text-xs text-gray-500">{{ inv.role }} &middot; expires {{ formatDate(inv.expires_at) }}</p>
                </div>
                <div class="flex gap-2">
                    <button @click="resendInvite(inv)" class="text-xs text-indigo-600 hover:text-indigo-700">Resend</button>
                    <button @click="cancelInvite(inv)" class="text-xs text-red-600 hover:text-red-700">Cancel</button>
                </div>
            </div>
        </div>
    </div>
...
async function fetchInvites(): Promise<void> {
    try {
        ...
        pendingInvites.value = (data.data ?? data).filter((i: Invite & { accepted_at?: string }) => !i.accepted_at);
...
async function removeMember(m: Member): Promise<void> {
    ...
    } catch { notifications.error('Failed to remove member.'); }
}
...
async function resendInvite(inv: Invite): Promise<void> {
    ...
    } catch { notifications.error('Failed to resend.'); }
}
...
async function cancelInvite(inv: Invite): Promise<void> {
    ...
    } catch { notifications.error('Failed to cancel invitation.'); }
}
