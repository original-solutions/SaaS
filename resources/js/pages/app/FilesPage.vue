<template>
    <div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div v-else-if="files.length === 0" class="p-6 text-center text-sm text-gray-500">No files uploaded yet.</div>
            ...
            <table v-else class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    <tr v-for="file in files" :key="file.id" class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500">{{ formatDate(file.created_at) }}</td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <button @click="downloadFile(file.id)" class="text-sm text-indigo-600 hover:text-indigo-800">Download</button>
                            <button v-if="tenantStore.canWrite" @click="deleteFile(file.id)" class="text-sm text-red-600 hover:text-red-800">Delete</button>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <div v-if="pagination.lastPage > 1" class="px-4 py-3 border-t border-gray-200 flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    <button :disabled="pagination.currentPage <= 1" @click="loadFiles(pagination.currentPage - 1)" class="px-3 py-1 text-sm border rounded-md disabled:opacity-50">Previous</button>
                    <button :disabled="pagination.currentPage >= pagination.lastPage" @click="loadFiles(pagination.currentPage + 1)" class="px-3 py-1 text-sm border rounded-md disabled:opacity-50">Next</button>
                </div>
            </div>
        </div>
    </div>
