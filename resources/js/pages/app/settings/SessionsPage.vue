        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="p-6 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600">Manage your active sessions across devices. If you notice any unfamiliar sessions, revoke them immediately.</p>
                    </div>
            ...
            <ul v-else class="divide-y divide-gray-200">
                <li v-for="session in sessions" :key="session.id" class="p-4 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-900">{{ session.device_name }}</p>
                            <p class="text-xs text-gray-500">
                                {{ session.ip_address }} &middot; Last active {{ formatRelativeTime(session.last_active_at) }}
                            </p>
                        </div>
                    </div>
