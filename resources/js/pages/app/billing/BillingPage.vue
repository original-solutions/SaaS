    ...
    <div>
        ...
        <div v-if="isReadOnly" class="mb-6 rounded-md bg-amber-50 border border-amber-200 p-4">
            <div class="flex items-center gap-3">
                ...
                <router-link to="/billing/plans" class="ml-auto px-3 py-1.5 text-xs font-medium bg-amber-600 text-white rounded-md hover:bg-amber-700">
                    Upgrade
                </router-link>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <template v-else-if="subscription">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <p class="text-xl font-bold text-gray-900">{{ subscription.plan?.name ?? 'Plan' }}</p>
                            <p class="text-sm text-gray-500 capitalize">{{ subscription.status }}</p>
                        </div>
                        <span :class="statusBadgeClass(subscription.status)" class="inline-flex items-center rounded-full px-3 py-1 text-xs font-medium">
                            {{ subscription.status }}
                        </span>
                    </div>
                    <dl class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <dd class="text-gray-900">{{ formatDate(subscription.current_period_start) }} — {{ formatDate(subscription.current_period_end) }}</dd>
                        </div>
                        <div v-if="subscription.grace_period_end" class="flex justify-between">
                            <dt class="text-gray-500">Grace period ends</dt>
                            <dd class="text-red-600">{{ formatDate(subscription.grace_period_end) }}</dd>
                        </div>
                    </dl>
                </template>
                <div v-else class="text-sm text-gray-500">
                    <p>No active subscription.</p>
                    <router-link to="/billing/plans" class="text-indigo-600 hover:text-indigo-800 font-medium">View available plans &rarr;</router-link>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    <router-link to="/billing/plans" class="flex items-center justify-between py-2 text-sm text-gray-700 hover:text-gray-900">
