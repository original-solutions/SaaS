...
- `last_used_at` (timestamp, nullable)
- `revoked_at` (timestamp, nullable)
- `revocation_reason` (string, nullable)  *(e.g., logout, password_reset, refresh_reuse_detected, admin_revoked)*
- `created_at`, `updated_at`
...
- `user_id`
- `key` (string)
- `channels` (json) *(e.g., {"in_app": true, "email": false})*
- `created_at`, `updated_at`
...
#### `webhook_events`
- `id`
- `provider` (string) *(stripe, twilio, vapi, etc.)*
...
- Dedupe by (provider, event_id) *(unique index)*
- Process via queue
- Safe retries
